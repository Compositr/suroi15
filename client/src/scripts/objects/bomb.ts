...
export class Bomb extends GameObject<ObjectCategory.Bomb> {
    override readonly type = ObjectCategory.Bomb;

    private readonly image = new SuroiSprite("airstrike_bomb");

    private scaleAnim?: Tween<Vector>;

    private fallSound?: GameSound;

    constructor(game: Game, id: number, data: Required<ObjectsNetData[ObjectCategory.Bomb]>) {
        super(game, id);
    ...
    override updateFromData(data: ObjectsNetData[ObjectCategory.Bomb], isNew = false): void {
        if (data.full) {
